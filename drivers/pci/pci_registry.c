...
const char *pci_device_name(uint32_t vendor_id, uint32_t device_id) {
    // Look up the device name based on vendor and device IDs
    return "Unknown Device";
}

const char *pci_class_name(uint32_t class_code, uint32_t subclass, uint32_t prog_intf) {
    // Look up the class name based on class, subclass, and programming interface
